...
int main( void ) {
  ...
  if (  pthread_create( &W2, NULL,  Writer, (void *)"1" ) != 0)
      printf("Error creating thread\n");
  
  if (  pthread_create( &R1, NULL,  Reader, (void *)"1" ) != 0)
      printf("Error creating thread\n");
  if (  pthread_create( &R2, NULL,  Reader, (void *)"1" ) != 0)
      printf("Error creating thread\n");
  if (  pthread_create( &R3, NULL,  Reader, (void *)"1" ) != 0)
      printf("Error creating thread\n");
...
void *Reader( void * num)
{
  ...
  for (i=1; i<= 3; i++){
     ...
     reader_unlock( semid );
     sleep(1);   /* note: sleep makes the entire process sleep  */
  }
  pthread_exit(0);
}
