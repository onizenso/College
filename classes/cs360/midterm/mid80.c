...
volatile sig_atomic_t saved_value;
...
int main(int argc, char **argv){ 
    ...
    int genNum=420;
    ballNum = atoi(argv[1]);
    struct { long type; char text[1000]; } rotater;
    struct sigaction sa;
    ...
    if(robot == 0){ /* Robot's code */      
        ...
        saved_value = 0;
        while(1){
            if(*shared > saved_value){
                printf("The robot is adjusting. \n");
                usleep(750000);
                saved_value++;
            }
    ...
    if(getpid() == ballThrower){
        ...
        while(i < ballNum){
            received = msgrcv(mq_id, &rotater, sizeof(rotater), 0, IPC_NOWAIT);
            
            strcpy(msgRec, rotater.text);
            printf("# of messages: %d\n", received);

            if(received > 0){
                printf("The %s \n", msgRec);
                //*shared+=1;
                printf("MQID is %d, plus IPCKEY is %d\n", mq_id, ipckey); 
            }
            else{
                printf("Fucking error asshole\n");
                //printf("MQID is %d, plus IPCKEY is %d\n", mq_id, ipckey); 
            }
            printf("Ball Thrown...\n");
            i++;
        ...
        printf("MQID = %d and IPCKEY = %d \n", mq_id, ipckey); 
        
        // REMOVING MESSAGE QUEUE(S) FROM SHARED MEMORY --//
        // ----------------------------------------------//
...
// REMOVING SEMAPHORES FROM SHARED MEMORY --------//
//-----------------------------------------------//
/*    
    if ((semctl(ipckey, rotateSMN, IPC_RMID)) < 0)
    {
          perror("semctl");
          exit(EXIT_FAILURE);
    } 
