...
--
   ...
   begin
      for Item in 1..Get_Size (Links) loop
         ...
         (  Data_Base  => Storage.Data_Base,
            ...
            Dependant  => Dependant,
            Referent   => Get_Key (Storage, Ref (Links, Item)).ID
         );
      end loop;
   end Create_Links;
   ...
   begin
      ...
      Execute (Storage.Data_Base);
      Fetch (Command);
      Class      := To_Unbounded_String (Value (Command, 1));
      Data       := To_Unbounded_String (Value (Command, 2));
      Parameters := To_Unbounded_String (Value (Command, 3));
      Clear (Command);
   exception
      when End_Error =>
         raise;
   ...
   begin
      ...
      Append (Command, Get_New_Parent_Key (Storage.all, Parent).ID);
   ...
   exception
      ...
      when Data_Error | Use_Error | Name_Error =>
         raise;
      when SQL_Error =>
         Raise_Exception
         (  Data_Error'Identity,
   ...
   begin
      ...
      begin
         ...
         or else
            Has_Dependants (Storage.Data_Base, Backward_Links_Table, ID)
         );
      end;
   end Has_Dependants;
