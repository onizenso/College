...
package body APQ.Common is
      ...
      case Engine_Of (Data_Base.Connection.all) is
         when Engine_PostgreSQL =>
            declare
               ...
               Object : Connection_Type renames
                           Connection_Type (Data_Base.Connection.all);
            begin
               if Is_Trace (Object) then
                  APQ.PostgreSQL.Client.Close_DB_Trace (Object);
               end if;
            end;
         when Engine_MySQL =>
            declare
            ...
            begin
               if Is_Trace (Object) then
                  APQ.MySQL.Client.Close_DB_Trace (Object);
               end if;
            end;
         when others =>
            null;
      ...
      case Server_Type is
         when Engine_PostgreSQL =>
            Data_Base.Connection :=
               new PostgreSQL.Client.Connection_Type;
         when Engine_MySQL =>
            Data_Base.Connection :=
               new MySQL.Client.Connection_Type;
         when others =>
            Raise_Exception
            (  Use_Error'Identity,
               (  Database_Type'Image (Server_Type)
               &  " data bases are not yet supported by APQ"
            )  );
      end case;
   ...
   begin
      case Engine_Of (Data_Base.Connection.all) is
         when Engine_PostgreSQL =>
            APQ.PostgreSQL.Client.Open_DB_Trace
         ...
         when Engine_MySQL =>
            APQ.MySQL.Client.Open_DB_Trace
   ...
   begin
      case Data_Base.Shared is
         when Fully | Read_Write =>
            Raise_Exception
            (  Use_Error'Identity,
               "Read (but write) transaction required for Table_Exists"
            );
         when Read_Only =>
            null;
      end case;
