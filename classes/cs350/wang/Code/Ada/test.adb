-- filename: test.adb (filename must match outer procedure name)
-- compile only:            $ gcc -c test.adb
-- compile and link:        $ gnatmake test.adb
-- execute:                 $ ./test
...
procedure Test is                 -- outermost procedure is entry point (main) 

  --  start of declaration section 
  package Flt_IO is new Ada.Text_IO.Float_IO(FLOAT);  -- package is like class
  use Flt_IO;

  str : STRING(1..5);             -- a string of 5 characters
  z : FLOAT;
  b : BOOLEAN;                   -- values are true/false only
  I : INTEGER;                   -- 32-bit signed int  Ada is case-insensitive 
  n:  NATURAL := 0;              -- NATURAL = {0...2147483647}            
  DOZEN : constant := 12;        -- static literal constant
  ...
  -- demonstrate parameter passing, file I/O, scoping, coercions 
  ...
  begin
    ...
    i := abs((-((2**3)+10)-3)/2);             -- test arithmetic operations
    i := 25 mod DOZEN;    
    i := 25 rem DOZEN;  
    New_Line;                       
    ...
    i := -1;
    n := i;      -- INT and NATURAL are both integers - so will compile w/
                 -- a warning but will raise a range error exception at runtime 
