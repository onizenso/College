...
void List<T>::add(const T &value)
{
  ...
  try {
    if ( count >= MAX )
        throw RangeError (__FILE__, __LINE__, count);
  }
...
void List<T>::print()
{
  ...
  // FYI: this try block throws an exception from the C++ standard exception library 
  try 
  {
     if ( count >= MAX )
...
{
  // add a try and catch block here
  // to check the value of count before sorting
...
{
  // add a try and catch block here
  // to check the value of count before displaying 
